                ...
                Data?.map(card => (
                    <Link to={{screen: "Post", params: {user: card, title: card.name}}}>
                        <View className="w-44 relative border-[1px] border-black/5 rounded-sm dark:bg-slate-300/10">
                            ...
                            <View className="p-2">
                                <Text numberOfLines={1} className="text-base font-semibold -mb-1 dark:text-white">
                                    {card.service}
                                </Text>

                                <Text numberOfLines={1} className="text-sm text-gray-500 dark:text-white/70">
                                    De {card.location}
                                </Text>
                                ...
                                <View className="flex flex-row py-2 justify-left items-left gap-x-2">
                                    ...
                                    />
                                    :
                                    <Ionicons name="person-circle-outline" size={32} color={colorScheme === 'dark'? "#fff" : "#000"} />
                                    }
                                    <View>
                                        <Text className="text-black dark:text-white">{card.name}</Text>
                                        {
                                            card.stars?

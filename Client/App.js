  ...
  return (
    <Tab.Navigator
      ...
      screenOptions={{
        ...
        headerLeft: () => (
          <Image style={{marginLeft: 15, resizeMode: "contain"}} className="h-12 w-12" source={require('./assets/logo.png')}/>
        ),
        headerRight: () => (
          <Ionicons onPress={toggleColorScheme} style={{marginRight: 20}} name="moon-outline" size={30} color={colorScheme === "dark"? "#fff" : "#000"} />
        ),
        tabBarStyle: {
          borderTopWidth: 0,
          backgroundColor: colorScheme === "dark"? DarkModeColor : LightModeColor,
    ...
    >
      <Tab.Screen name="Inicio" component={Index} options={{
        tabBarLabel: 'Inicio',
        tabBarIcon: (props) => (
          <HomeIcon color={props.color} />
        ),
      }}/>
      <Tab.Screen name="Buscar" component={Search} options={({route}) => ({
        tabBarLabel: 'Buscar',
        tabBarIcon: (props) => (
...
function App() {
  ...
  return (
    ...
    <Stack.Navigator>
      ...
      <Stack.Screen name="Message" component={Message} options={({ route }) => ({ title: route.params.title })}/>
    </Stack.Navigator>
    </NavigationContainer>
    </>
  );
